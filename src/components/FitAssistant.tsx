...
interface FitAssistantProps {
  productType: 'tops' | 'bottoms' | 'dresses' | 'outerwear' | 'shoes' | 'accessories';
}

const FitAssistant: React.FC<FitAssistantProps> = ({ productType }) => {
  ...
  const [fitPreference, setFitPreference] = useState<'snug' | 'regular' | 'loose'>('regular');
  ...
  const fieldLabels: Record<string, { label: string; unit: string; placeholder: string }> = {
                ...
                <div className="flex space-x-2">
                  {['snug', 'regular', 'loose'].map((fit) => (
                    <button
                      ...
                      onClick={() => setFitPreference(fit as 'snug' | 'regular' | 'loose')}
                    >
