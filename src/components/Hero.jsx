    ...
    <section className="relative h-[80vh] md:h-[85vh] bg-ju4u-gray overflow-hidden">
      <div className="container max-w-7xl mx-auto h-full flex flex-col md:flex-row">
        <div className="w-full md:w-1/2 flex flex-col justify-center px-4 md:px-8 py-12 md:py-0 z-10">
          <span className="text-ju4u-coral font-medium uppercase tracking-wider mb-2 animate-fade-in">New Collection</span>
          <h1 className="font-rubik text-4xl md:text-5xl lg:text-6xl font-bold mb-4 animate-fade-in leading-tight">
        ...
        <div className="w-full md:w-1/2 relative h-[40vh] md:h-full">
          ...
          <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1581044777550-4cfa60707c03?q=80&w=1972')] bg-cover bg-center transform transition-transform duration-700 hover:scale-105"></div>
      ...
      <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex space-x-2">
        <span className="h-2 w-8 rounded-full bg-ju4u-coral animate-pulse"></span>
        <span className="h-2 w-2 rounded-full bg-gray-300 hover:bg-gray-400 cursor-pointer transition-all duration-300"></span>
        <span className="h-2 w-2 rounded-full bg-gray-300 hover:bg-gray-400 cursor-pointer transition-all duration-300"></span>
      </div>
      
      {/* Decorative element for luxury feel */}
      <div className="absolute top-16 right-16 hidden lg:block">
        <div className="w-24 h-24 border-2 border-ju4u-coral/30 rounded-full"></div>
      </div>
    </section>
  );
};
